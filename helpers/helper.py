""" Helpening module. Searching by tags
and create string of results """
...
def login_to_website(driver, email, password) -> None:
    """ Login to the site https://belurk.online/ using email and password."""
...
def navigate_to_proxies(driver):
    """  Button selection 'ipv4-shared Прокси'"""
    find_section = (WebDriverWait(driver, 5).
...
def navigate_to_table(driver) -> str:
    """  Searching table and create result """
    table = (WebDriverWait(driver, 5).
...
def search_create_result(rows) -> str:
    """ Searching in table and create string for result """
    data_cells = []
    for row in rows:
        cells = row.find_elements(By.TAG_NAME, 'td')
        if cells:
            expiration_date = [cell.text for cell in cells]
            data_cells.append(str(f'{expiration_date[5]} '
                                  f'- {expiration_date[9]}'))
    return '\n'.join(data_cells)
